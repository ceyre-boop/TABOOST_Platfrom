...
function updateScoreAndLevels() {
    ...
    if (scoreBadgeEl) {
        // UK: no cash back / rank boost rewards — score badge shows the score only
    ...
    // UK: no rank boost — keep plain scale markers, no celebratory "unlock" badge
    if (prosItem) {
    ...
    const diamonds = myData.diamonds || 0;
    // Update Dollars display (main value).
    // UK sheet has no "Est Rev" column, so fall back to the diamond rate (same 0.005 used elsewhere).
    ...
    if (proBonusBadge) {
        ...
        // UK: no agency cash bonus / pro halo — never qualifies
        const qualifiesForPro = false;
    ...
    // UK: Agency Cash Bonus removed from Revenue Streams (US-only reward).
    // Ensure no bonus halo lingers on the score section.
    if (scoreSection) {
        scoreSection.classList.remove('pro-active');
